...
#include "Car/SportsCar/TCSportsCar.h"
...
{
	GetChaosVehicleMovement()->ChassisHeight = 144.0f; //무게 중심 높이 낮을수록 회전 안정적 코니 기울기 작음
	GetChaosVehicleMovement()->DragCoefficient = 0.31f; // 공기 저항 낮을수록 최고속도 높음 가속 빠름

	// Set up the wheels
	GetChaosVehicleMovement()->bLegacyWheelFrictionPosition = true;
	GetChaosVehicleMovement()->WheelSetups.SetNum(4);

	GetChaosVehicleMovement()->WheelSetups[0].WheelClass = UTCSportsCarWheelFront::StaticClass();
	...
	GetChaosVehicleMovement()->WheelSetups[1].WheelClass = UTCSportsCarWheelFront::StaticClass();
	...
	GetChaosVehicleMovement()->WheelSetups[2].WheelClass = UTCSportsCarWheelRear::StaticClass();
	...
	GetChaosVehicleMovement()->WheelSetups[3].WheelClass = UTCSportsCarWheelRear::StaticClass();
